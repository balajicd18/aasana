...
const Dashboard = () => {
  ...
  const [conformedBooking, setConformedBooking] = useState([])
...
useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setBookingList(response.data.result); // ← store bookings in state
      })
      .catch((error) => {
        console.error("Error fetching bookings:", error);
      });

      axios.get("https://assana-test.vercel.app/v1/nurse/get-confirmed-bookings", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      })
      .then((response) => {
        console.log("Full response:", response);
        setConformedBooking(response.data.result); // ← store bookings in state
      })
      .catch((error) => {
        console.error("Error fetching bookings:", error);
      });
...
const columns1 = [
  ...
  columnHelperPatient.accessor("patient_name", {
    cell: (info) => info.getValue(),
    header: () => (
      <span className="flex items-center">
        <User className="mr-2" size={16} /> Patient Name
...
columnHelper.accessor("booking_time", {
  ...
  columnHelperPatient.accessor("whatsapp_number", {
    cell: (info) => info.getValue(),
    header: () => (
      <span className="flex items-center">
        <Phone className="mr-2" size={16} />Whatsapp Number
...
const dataAllPatient = React.useMemo(() => conformedBooking, [conformedBooking]);
  ...
  return (
    <>
      <div className="flex flex-col min-h-screen  mx-auto py-12 px-4 sm:px-6 lg:px-8 bg-[]">
        <h1 className='font-bold text-2xl mt-10'>All Appointments</h1>
...
{/* all patient */}
        <h1 className='font-bold text-2xl mt-10'>Confirmed Appointments</h1>
